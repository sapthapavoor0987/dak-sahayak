...
// MediaRecorder Voice Assistant Integration
    ...
    try {
        ...
        mediaRecorder.onstart = () => {
            ...
            if (micBtn) {
                ...
                micBtn.title = "Click mic again to submit";
            }
            if (input) {
                input.value = '';
                input.placeholder = "🎙️ Recording... Speak now, then click mic to send";
            }
        ...
        if (input) input.placeholder = PLACEHOLDERS[currentLanguage] || PLACEHOLDERS['English'];
        showToast("⚠️ Microphone access failed. Please check microphone permissions in browser & Windows settings.");
    }
}
